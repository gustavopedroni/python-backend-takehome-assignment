...
def test_post_risk_profile_response(test_app, mocker, user_common_data_dict):
    ...
    response = test_app.post(
        "/v1/risk_profile/", data=json.dumps(user_common_data_dict)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("income", -120),
        ("income", None),
        ("marital_status", "singled"),
        ("marital_status", ""),
        ("marital_status", None),
        ("risk_questions", [2, -1, 1]),
        ("risk_questions", [1, 1, 1, 1]),
        ("risk_questions", [1, 1]),
        ("house", {"status": "owned"}),
        ("house", 2),
        ("house", {"ownership_status": "mortgage"}),
        ("vehicle", {"year_of_model": 2050}),
        ("vehicle", 2),
        ("vehicle", {"year": -3}),
    ],
)
