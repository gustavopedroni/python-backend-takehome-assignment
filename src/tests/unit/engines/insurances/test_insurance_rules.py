...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            lazy_fixture("user_data_without_house"),
        ),
        (
            personal.NotHaveIncomeRule,
            InsurancesLineKey.DISABILITY,
            lazy_fixture("user_data_without_income"),
        ),
        (
            vehicle.NotHaveVehicleRule,
            InsurancesLineKey.AUTO,
            lazy_fixture("user_data_without_vehicle"),
        ),
    ],
)
def test_dont_have_rule(rule_class, line_key, user_data, risk_score):
    ...
    score = rule.apply(data=user_data, score=risk_score.num_score)
...
def test_age_over_60_years(risk_score, user_data_age_over_60_years):
    ...
    for key in allowed_keys:
        ...
        score = rule.apply(data=user_data_age_over_60_years, score=risk_score.num_score)
...
def test_age_under_30_years(risk_score, user_data_age_under_30_years):
    ...
    for key in tuple(InsurancesLineKey):
        ...
        score = rule.apply(
            data=user_data_age_under_30_years, score=risk_score.num_score
        )
...
def test_age_between_30_40_years(risk_score, user_data_age_between_30_40_years):
    ...
    for key in tuple(InsurancesLineKey):
        ...
        score = rule.apply(
            data=user_data_age_between_30_40_years, score=risk_score.num_score
        )
...
def test_income_above_200k(risk_score, user_data_income_above_200k):
    ...
    for key in tuple(InsurancesLineKey):
        ...
        score = rule.apply(data=user_data_income_above_200k, score=risk_score.num_score)
...
def test_house_mortgaged(risk_score, user_data_house_mortgaged):
    ...
    for key in allowed_keys:
        ...
        score = rule.apply(data=user_data_house_mortgaged, score=risk_score.num_score)
...
def test_has_dependents(risk_score, user_data_with_dependents):
    ...
    for key in allowed_keys:
        ...
        score = rule.apply(data=user_data_with_dependents, score=risk_score.num_score)
...
def test_is_married(risk_score, user_data_married):
    ...
    score = rule.apply(data=user_data_married, score=risk_score.num_score)
...
def test_vehicle_produced_in_last_5_years(risk_score, user_data_vehicle_last_5_years):
    ...
    score = rule.apply(data=user_data_vehicle_last_5_years, score=risk_score.num_score)

    assert risk_score.num_score + 1 == score

    others_keys = tuple(
        [x for x in InsurancesLineKey if x is not InsurancesLineKey.AUTO]
    )

    for key in others_keys:
        rule = vehicle.VehicleProducedLast5YearsRule(line_key=key)
        score = rule.apply(
            data=user_data_vehicle_last_5_years, score=risk_score.num_score
        )
